...
$.get("users/getData").done(function(data) {
    ...
    for (var i = 0; i < data.length; i++) {
        var listCurrent = [data[i]['yl'], data[i]['kp'], data[i]['sp'], data[i]['zd'], data[i]['zg']];
        allList[data[i]['yl']] = [data[i]['nl'], data[i]['xq'], data[i]['kpspc'], data[i]['zdf'], data[i]['gzn'], data[i]['gzy'], data[i]['gzr']];
        getZhouYiList[data[i]['yl']] = [data[i]['gzn'], data[i]['gzy'], data[i]['zg']];
    ...
    option = {
        ...
        tooltip: {
            ...
            formatter: function(params) {
                ...
                res += '<br/>  干支 : ' + allData[4] + "-" + allData[5] + "-" + allData[6];
...
function getMoonPhase(getMoonList) {
    ...
    for (var i = 1; i < getMoonList.length; i++) {
        ...
        var prevlStr = getMoonList[i - 1][1].substr(getMoonList[i - 1][1].length - 2);
        var json = {};
        if (nlStr == "初八") {
            numMoon(json, '上弦', i);
        } else {
...
function getJieQi(getMoonList) {
    ...
    for (var i = 1; i < getMoonList.length; i++) {
        ...
        var prevYlyStr = getMoonList[i - 1][0].substr(getMoonList[i-1][0].indexOf("-")+1);
        var json = {};
        if (ylyStr == "02-19") {
            JieQi(json, '雨水', i);
        } else {
...
function getWeiZhiData(lastList) {
    ...
    for (var i = 0; i < 60; i++) {
        ...
        if(date!="6"&&date!="0"){
        var d=window.LunarCalendar.solarToLunar(parseInt(ylStr[0]),parseInt(ylStr[1]),parseInt(ylStr[2]));
         c.xq=date;
         c.gzn=d.GanZhiYear;
         c.gzy=d.GanZhiMonth;
         c.gzr=d.GanZhiDay;
