...
(function() {
	...
	setTimeout(function() {
		...
		var trEachTimer = setInterval(function() {
			...
			console.log(trIndex);
			var trHtml = $("#BIZ_hq_historySearch tbody tr:gt(0)").eq(trIndex*1-1).children('td');
			...
			var kpspc = ((parseFloat(td2) - parseFloat(td1)) / parseFloat(td2)) * 100;
			var ylStr=td0.split("-");
			var zyArray=window.LunarCalendar.solarToLunar(parseInt(ylStr[0]),parseInt(ylStr[1]),parseInt(ylStr[2]));
			...
			console.log(window.LunarCalendar.solarToLunar(parseInt(ylStr[0]),parseInt(ylStr[1]),parseInt(ylStr[2])));
			$.ajax({
						type: "post",
						timeout: 6000,
						async: true,
